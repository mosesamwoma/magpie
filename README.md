...
This builds the image (Python + ffmpeg), mounts `./downloads` on the host, and
serves the app on `http://localhost:5000`. Environment variables are read from
`backend/.env` if present (copy `backend/.env.example` there first) — it's
optional, sensible defaults apply without one.
